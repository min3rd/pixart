...
import {
  EditorPuppetWarpService,
  PuppetWarpState,
} from '../editor-puppet-warp.service';
import { EditorTransformService } from '../editor-transform.service';

export interface PuppetWarpBufferState {
...
export class CanvasPuppetWarpHandler {
  ...
  getState(): PuppetWarpBufferState {
    return {
    ...
    if (state.pins.length === 0) {
      for (let y = 0; y < this.originalRect.height; y++) {
        for (let x = 0; x < this.originalRect.width; x++) {
          ...
          const color = this.originalBuffer[srcIdx];
          if (color) {
            const destX: number = this.originalRect.x + x;
            const destY: number = this.originalRect.y + y;
            if (
